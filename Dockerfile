...
RUN git clone https://github.com/MATPOWER/matpower-extras.git /usr/src/matpower/extras
...
RUN pip3 install numpy scipy oct2py

ADD oct2pypower.py /usr/src/oct2pypower.py

WORKDIR /workspace

ENTRYPOINT ["ipython3", "-i", "--no-banner", "/usr/src/oct2pypower.py"]
